...
const FeatureFlagList = () => {
   ...
   const cachedData = queryClient.getQueryData(['hydrate-flags']) as Record<string,any>;

    const featureFlagKeys = Object.keys(cachedData)
    const featureFlagValuesTypes = Object.values(cachedData).map(value=> typeof value)
...
<div>
      ...
      {filteredFlags.map((flag,i) => (
        <div key={flag} className='flex'>
          <p>{flag} {`(${featureFlagValuesTypes[i]})`}</p>
